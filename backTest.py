...
# 感觉策略就给出每分钟该买啥就好了，权重大概率是要大于该分钟的交易的，那就直接就按当分钟交易的1/4就好
...
working_folder = 'D:\大学文档\MAFM\AI in fintech\project 3'
...
format1_dir = './data/train_data_format1_week_final.h5'
format2_dir = './data/train_data_format2_week_final.h5'
...
from HMM_strategy import handle_bar
...
class backTest:
    ...
    def backTest(self):
        ...
        for i in tqdm(range(len(keys))):  # minuts
            ...
            mask = np.abs(position_change) > .25*data_cur_min[:,4] # 成交量大于1/4的分钟级成交量的话
            position_change[mask] = (.25*data_cur_min[:,4]*np.sign(position_change))[mask]  # 可以是做空
            position_new = position_old + position_change
            # print(position_new)
            print(total_balance)

            # print(sum(abs(position_new*data_cur_min[:,0])))
            ...
            crypto_balance = np.sum(np.abs(position_new*average_price))  # 绝对值
